...
private:
    ...
    bool modifyAndroidTemplate(const std::string& buildDir, const ApkConfig& config);
    ...
    bool customizeJavaFiles(const std::string& buildDir, const ApkConfig& config);
    std::string generatePermissions(const ApkConfig& config);
    std::string generateServerConfig(const ApkConfig& config);
    std::string executeGradleBuild(const std::string& buildDir, const ApkConfig& config);
    std::string signApk(const std::string& unsignedApkPath, const ApkConfig& config, const std::string& keystorePath, const std::string& keystorePass, const std::string& keyAlias, const std::string& keyPass);
